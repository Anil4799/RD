...
class TestStudentServiceImpl {
	...
	{ 
		...
		collegeLocation=bean.getCollegeLocation();
		graduation=bean.getGraduation();
		graduationSpeciality=bean.getGraduationSpeciality();
		yearOfPassedOut=bean.getYearOfPassedOut();
		graduationMarks=bean.getGraduationMarks();
		twelveth=bean.getTwelveth();
		tenth=bean.getTenth();
		batchId=bean.getBatchId();
		employeeType=bean.getEmployeeType();
		coreSkill=bean.getCoreSkill();
		preferredStudentStream=bean.getPreferredStudentStream();
		assignedStream=bean.getAssignedStream();
		mentorName=bean.getMentorName();
		assignedLocation=bean.getAssignedLocation();
		relocation=bean.getRelocation();
		...
		assertNotNull(personalLocation);
		assertNotNull(collegeName);
		assertNotNull(collegeLocation);
		assertNotNull(graduation);
		assertNotNull(graduationSpeciality);
		assertNotNull(yearOfPassedOut);
		assertNotNull(graduationMarks);
		assertNotNull(twelveth);
		assertNotNull(tenth);
		assertNotNull(batchId);
		assertNotNull(employeeType);
		assertNotNull(coreSkill);
		assertNotNull(preferredStudentStream);
		assertNotNull(assignedStream);
		assertNotNull(mentorName);
		assertNotNull(assignedLocation);
